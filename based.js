...
if (!methodCodeQR && !methodCode && !fs.existsSync(`./${authFile}/creds.json`)) {
    do {
        ...
        const color1 = chalk.hex('#00F0FF'); // Cyan
        const color2 = chalk.hex('#191970'); // Blue
        const color3 = chalk.hex('#BC13FE'); // Deep Purple
        const color4 = chalk.hex('#0000B3D'); // Bright Blue
        const softText = chalk.hex('#00BFFF');

        const a = color1('╭━━━━━━━━━━━━━• ✧˚💎 𝚭𝚵𝚼𝚴𝚰 𝚩𝚰𝚮 💠˚✧ •━━━━━━━━━━━━━');
        const b = color1('╰━━━━━━━━━━━━━• ☾⋆₊✧ 𝖇𝖑𝖔𝖔𝖉𝖇𝖔𝖙 ✧₊⋆☽ •━━━━━━━━━━━━━');
        const linea = color2('   ✦━━━━━━✦✦━━━━━━༺💧༻━━━━━━༺💧༻━━━━━━✦✦━━━━━━✦');
        const sm = chalk.bold.hex('#F8F1E9')('SELEZIONE METODO DI ACCESSO ✦');
        ...
        const istruzioni = [
            ...
            softText.italic(''),
            color1.italic('                   by Endy'),
        ];
        const prompt = chalk.hex('#F0F4F5').bold('\n⌯ Inserisci la tua scelta ---> ');
...
${chalk.hex('#FF00FF')('   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')}
${chalk.hex('#F5F0E6').bold('⚠️  Sono ammessi solo i numeri')} ${chalk.bold.cyan('1')} ${chalk.hex('#FF5E62').bold('o')} ${chalk.bold.cyan('2')}
${chalk.hex('#BC13FE')('┌─⭓ Nessuna lettera o simbolo')}
${chalk.hex('#00E5FF')('└─⭓ Copia il numero dell\'opzione desiderata e incollalo')}
...
async function connectionUpdate(update) {
    ...
    if (connection === 'open') {
        ...
        if (!global.isLogoPrinted) {
            const logoColors = [
                '#BC13FE', '#000B3D', '#00D4FF', '#FF00CC', '#FF0099',
                '#F2EDE4', '#00E5FF', '#002366', '#87CEEB', '#FAF6EF', '#E500FF'
            ];
            const varebot = [`███████╗███████╗██╗   ██╗███╗   ██╗ ██████╗ 
╚══███╔╝██╔════╝╚██╗ ██╔╝████╗  ██║██╔═══██╗
  ███╔╝ █████╗   ╚████╔╝ ██╔██╗ ██║██║   ██║
...
╚══════╝╚══════╝   ╚═╝   ╚═╝  ╚═══╝ ╚═════╝ 
    ...
    if (connection === 'close') {
        ...
        if (reason === DisconnectReason.badSession && !global.connectionMessagesPrinted.badSession) {
            ...
            console.log(chalk.bold.hex('#000B3D')(`╭⭑⭒━━━✦❘༻ ⚠️ CONNESSIONE SOSTITUITA ༺❘✦━━━⭒⭑\n┃ È stata aperta un'altra sessione. \n╰⭑⭒━━━✦❘༻☾⋆⁺₊✧ 𝖇𝖑𝖔𝖔𝖉𝖇𝖔𝖙 ✧₊⁺⋆☽༺❘✦━━━⭒⭑`));
            ...
            console.log(chalk.bold.hex('#F2EDE4')(`\n╭⭑⭒━━━✦❘༻ ⌛ TIMEOUT CONNESSIONE ༺❘✦━━━⭒⭑\n┃ 🔄 RICONNESSIONE IN CORSO...\n╰⭑⭒━━━✦❘༻☾⋆⁺₊✧ 𝖇𝖑𝖔𝖔𝖉𝖇𝖔𝖙 ✧₊⁺⋆☽༺❘✦━━━⭒⭑`));
...
async function connectSubBots() {
    ...
    if (!existsSync(subBotDirectory)) {
        console.log(chalk.bold.hex('#00D4FF')('💠 vare ✧ bot: Nessun Sub-Bot trovato.'));
        try {
    ...
    try {
        ...
        if (subBotFolders.length === 0) {
            console.log(chalk.bold.hex('#F2EDE4')('- 🌑 | Nessun subbot collegato'));
            return;
        }
...
global.reload = async (_ev, filename) => {
    if (pluginFilter(filename)) {
        ...
        if (filename in global.plugins) {
            if (existsSync(dir)) conn.logger.info(chalk.hex('#F2EDE4')(`✅ AGGIORNATO - '${filename}'`));
            else {
                conn.logger.warn(chalk.hex('#FF0099')(`🗑️ ELIMINATO: '${filename}'`));
                return delete global.plugins[filename];
            }
        } else conn.logger.info(chalk.hex('#F2EDE4')(`🆕 NUOVO PLUGIN: '${filename}'`));
